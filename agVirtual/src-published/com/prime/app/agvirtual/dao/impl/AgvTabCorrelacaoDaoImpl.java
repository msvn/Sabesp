...
public class AgvTabCorrelacaoDaoImpl extends GenericHibernateJpaDao<AgvTabCorrelacao, Long> implements AgvTabCorrelacaoDao {
	...
	public List<AgvTabCorrelacao> findByAtendimento(Long codAuto, boolean b) {
		List<AgvTabCorrelacao> result = null;
		try{
		org.hibernate.Query query = getHibernateSession().createQuery("" +
				"from AgvTabCorrelacao e where e.agvTabAutoatendimento.cdAutoatendimento =:cdAutoatendimento");
    	query.setLong("cdAutoatendimento",codAuto );
    	 result = query.list();            
        } catch(Exception e) {
        	logger.error("Erro na busca de link de correlação - erro na carga de dados" + e.getMessage());
             e.printStackTrace();
        } 
	...
	public void save(AgvTabAutoatendimento autoAtend ,List<AgvTabCorrelacao> listaFuncionalidadesARemover){
		...
		for (Iterator<AgvTabCorrelacao> iterator = autoAtend.getListaCorrelacao().iterator(); iterator.hasNext();) {
